<template>
  <div class="space-y-6">
      ...
      <div class="pt-4 mt-2">
        <form @submit.prevent="handleAddMember" class="flex gap-2">
          <input v-model="newMemberPhone" type="text" placeholder="Add by phone..."
            class="flex-1 text-sm border border-slate-200 dark:border-slate-700 bg-transparent dark:text-slate-100 rounded-lg px-3 py-2 focus:border-slate-400 dark:focus:border-slate-500 focus:ring-0 outline-none transition-colors"
            required />
    ...
    <BaseDialog :is-open="isRemoveMemberDialogOpen" title="Remove Member"
      :message="`Are you sure you want to remove ${memberToRemove?.name} from this group?`" confirm-text="Remove Member"
      cancel-text="Cancel" confirm-variant="danger" icon="pi-user-minus" :is-loading="isRemoving"
      @close="isRemoveMemberDialogOpen = false" @confirm="executeRemoveMember" />
  </div>
</template>
