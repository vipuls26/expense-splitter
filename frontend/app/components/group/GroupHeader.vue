<template>
    <div
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl phone-lg:text-xl tablet:text-3xl font-bold text-slate-900 dark:text-slate-100">{{
                            groupStore.currentGroup?.name }}</h1>
                        <button v-if="isOwner" @click="startEditing"
                            class="text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors p-1"><i
                                class="pi pi-pencil text-sm"></i></button>
                    </div>
                    <p class="text-xs phone-lg:text-sm text-slate-500 dark:text-slate-400 mt-1">{{ groupStore.currentGroup?.description
        ...
        <!-- edit group module -->
        <div v-if="isEditing" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 transition-opacity">
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl max-w-md w-full max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up transition-colors">
                ...
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">Edit Group Details</h3>
                    <button @click="isEditing = false" class="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
