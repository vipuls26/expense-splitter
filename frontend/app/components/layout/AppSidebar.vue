<template>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto py-6 px-3 space-y-1">
      ...
      >
        <i :class="['pi text-lg', item.icon, (item.href === '/' ? $route.path === '/' : $route.path.startsWith(item.href)) ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500']"></i>
        {{ item.name }}
      </NuxtLink>
    </div>
    ...
    <div class="p-4 border-t border-slate-100 dark:border-slate-800">
      <div class="flex items-center gap-3 px-3 py-2">
        <div class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center font-bold text-sm">
          {{ authStore.user?.name?.charAt(0).toUpperCase() || 'U' }}
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{{ authStore.user?.name || 'User' }}</p>
