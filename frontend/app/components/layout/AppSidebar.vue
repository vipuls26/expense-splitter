<template>
  <aside
    ...
    <div class="flex-1 overflow-y-auto py-6 px-3 space-y-1">
      <NuxtLink v-for="item in navigation" :key="item.name" :to="item.href" @click="$emit('close')"
        class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors" :class="[
          (
            item.href === '/'
              ? $route.path === '/'
