<template>
  <div class="space-y-6">
    <div v-if="settlementStore.isLoading && !settlementStore.balances.length" class="space-y-8">
      <section>
        <BaseSkeleton width="8rem" height="1rem" className="mb-4" />
        <div class="space-y-3">
          <div v-for="i in 2" :key="i"
            class="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
    ...
    <div v-else-if="!settlementStore.balances.length"
    ...
    <div v-else class="space-y-8">
      ...
      <section v-if="myDebts.length > 0 || debtsToMe.length > 0">
        ...
        <div class="space-y-3">
          <div v-for="debt in myDebts" :key="`debt-${debt.to.id}`"
            ...
            <BaseButton @click="openSettleModal(debt)" size="sm"
              class="bg-emerald-600 hover:bg-emerald-700 text-white border-none">
              Settle Up
            </BaseButton>
          </div>

          <div v-for="debt in debtsToMe" :key="`credit-${debt.from.id}`"
