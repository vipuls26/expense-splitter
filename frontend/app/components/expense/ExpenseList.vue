  ...
  <div
    ...
    <div
      ...
      <div class="min-w-0 pr-2">
        <h2 class="text-lg phone-lg:text-xl font-bold text-slate-900 dark:text-slate-100 truncate">
          Group Expenses
        </h2>
        <p class="text-xs phone-lg:text-sm text-slate-500 dark:text-slate-400 mt-0.5 phone-lg:mt-1 truncate">
    ...
    <div v-if="expenseStore.isLoading && expenseStore.expenses.length === 0"
      class="divide-y divide-slate-100 dark:divide-slate-800">
      <div v-for="i in 3" :key="i"
        class="p-4 tablet:p-6 flex flex-col tablet:flex-row tablet:items-center justify-between gap-4">
        ...
        <div class="flex items-start gap-4">
          ...
          <div>
            <div class="flex items-center gap-2 mb-1">
              ...
              <span v-if="expense.expense_category"
                ...
                :style="{ backgroundColor: expense.expense_category.color || '#94a3b8' }" title="Expense Category">
            ...
            <div class="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
              ...
              <span>paid</span>
              <span class="font-semibold text-emerald-600 dark:text-emerald-400">₹{{ Number(expense.amount) }}</span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          <div class="text-sm text-right">
            ...
            <span :class="[
              ...
              ₹{{ getUserShare(expense, authStore.user?.id).toFixed(2) }}
            </span>
          </div>

          <button v-if="expense.paid_by.id === authStore.user?.id || isOwner" @click="handleDelete(expense.id)"
    ...
    <!-- basedialogbox -->
    <BaseDialog :is-open="isDeleteDialogOpen" title="Delete Expense"
      message="Are you sure you want to delete this expense? This action cannot be undone."
      confirm-text="Delete Expense" cancel-text="Cancel" confirm-variant="danger" icon="pi-trash"
      :is-loading="isDeleting" @close="isDeleteDialogOpen = false" @confirm="executeDelete" />
  </div>
</template>
...
import { getUserShare } from "#imports";
